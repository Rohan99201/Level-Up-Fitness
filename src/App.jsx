...
function WorkoutPage({ user, isAdmin }) {
  ...
  useEffect(()=>{ load() },[clientId])
  useEffect(()=>{ loadLogs() },[clientId,week])
  ...
  const saveExercise=async()=>{
    ...
    try {
      ...
      setFormMsg('✓ Saved'); setShowAddModal(false); setEditEx(null)
  ...
  const deleteExercise=async(ex)=>{
    ...
    if (isDemo||clientId==='demo'){setProgram(prev=>prev.filter(p=>p.id!==ex.id));return}
    try { await sbDelete('workout_programs',ex.id); await load() } catch(e){alert(e.message)}
  }

  const openEdit=(ex)=>{ setEditEx(ex); setExForm({day_number:ex.day_number,workout_type:ex.workout_type,exercise_name:ex.exercise_name,set_rep:ex.set_rep||'',tempo:ex.tempo||'3010',rest_seconds:ex.rest_seconds||120,sets:ex.sets||3,video_url:ex.video_url||''}); setShowAddModal(true) }
  ...
  const ExForm=(
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
        ...
        <div><span style={S.lbl}>Rest (seconds)</span><input type="number" value={exForm.rest_seconds} onChange={e=>setExForm(p=>({...p,rest_seconds:parseInt(e.target.value)||120}))} style={S.inp}/></div>
        ...
        <div style={{ gridColumn:'1/-1' }}><span style={S.lbl}>Video URL (optional)</span><input value={exForm.video_url} onChange={e=>setExForm(p=>({...p,video_url:e.target.value}))} placeholder="https://youtu.be/…" style={S.inp}/></div>
      </div>
      <MsgBox msg={formMsg}/>
          ...
          {showDayPicker&&(
            <div style={{ background:'#fff', border:`1px solid ${C.border}`, borderRadius:10, marginTop:6, overflow:'hidden' }}>
              {days.map((d,i)=>{ const t=program.find(p=>p.day_number===d)?.workout_type||''; return (
                <button key={d} onClick={()=>{setDayIdx(i);setShowDayPicker(false)}} style={{ width:'100%', padding:'13px 16px', background:dayIdx===i?C.accentL:'#fff', border:'none', borderBottom:`1px solid ${C.border}`, textAlign:'left', cursor:'pointer', fontSize:14, color:dayIdx===i?C.accent:'#111', fontWeight:dayIdx===i?600:400 }}>Day {d} — {t}</button>
              )})}
            </div>
          )}
        </div>
...
function NutritionPage({ user, isAdmin }) {
  ...
  useEffect(()=>{ load() },[clientId,dietType])
  ...
  const saveMealItem=async()=>{
    ...
    try {
      ...
      setFormMsg('✓ Saved'); setShowAddModal(false); setEditItem(null)
  ...
  const deleteMealItem=async(item)=>{
    ...
    if (isDemo||clientId==='demo'){setMeals(prev=>prev.filter(m=>m.id!==item.id));return}
    try { await sbDelete('meal_plans',item.id); await load() } catch(e){alert(e.message)}
  }

  const openEdit=(item)=>{ setEditItem(item); setMealForm({meal_number:item.meal_number,meal_name:item.meal_name||'MEAL 1',food_name:item.food_name,category:item.category||'Proteins',qty_g:item.qty_g||0,calories:item.calories||0,protein_g:item.protein_g||0,fat_g:item.fat_g||0,carbs_g:item.carbs_g||0,fibre_g:item.fibre_g||0}); setShowAddModal(true) }
    ...
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
        ...
        {[['Calories','calories'],['Protein (g)','protein_g'],['Fat (g)','fat_g'],['Carbs (g)','carbs_g'],['Fibre (g)','fibre_g']].map(([lbl,k])=>(
          <div key={k}><span style={S.lbl}>{lbl}</span><input type="number" value={mealForm[k]} onChange={e=>setMealForm(p=>({...p,[k]:parseFloat(e.target.value)||0}))} step=".1" style={S.inp}/></div>
        ))}
      </div>
      <MsgBox msg={formMsg}/>
      <div style={{ display:'flex', gap:9, marginTop:14 }}>
        <Btn onClick={saveMealItem} style={{ flex:1 }}>{editItem?'Save changes':'Add food item'}</Btn>
        <Btn variant="secondary" onClick={()=>{setShowAddModal(false);setEditItem(null)}} style={{ flex:1 }}>Cancel</Btn>
      </div>
    </div>
  )
  ...
  return (
    <div style={S.page}>
      <div style={{ marginBottom:18 }}><div style={S.h1}>Nutrition</div><div style={{ fontSize:13, color:C.gray }}>Personalised meal guide</div></div>
      ...
      {showAddModal&&<Modal title={editItem?`Edit: ${editItem.food_name}`:'Add food item'} onClose={()=>{setShowAddModal(false);setEditItem(null)}}>{MealItemForm}</Modal>}
      ...
      <div style={S.card}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14, flexWrap:'wrap', gap:8 }}>
          ...
          <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
            ...
            {isAdmin&&<Btn variant="green" small onClick={()=>{setEditItem(null);setShowAddModal(true)}}>+ Add item</Btn>}
          </div>
        </div>
        ...
        {Object.keys(grouped).length===0?(
          ...
          <div key={mealName} style={{ marginBottom:18 }}>
            <div style={{ fontWeight:600, fontSize:13, color:C.accent, marginBottom:8 }}>
              {mealName} <span style={{ fontSize:11, color:C.gray, fontWeight:400 }}>{items.reduce((s,i)=>s+(+i.calories||0),0).toFixed(0)} kcal · {items.reduce((s,i)=>s+(+i.protein_g||0),0).toFixed(1)}g protein</span>
            </div>
            {isMobile?(
              items.map((item,i)=>(
                <div key={item.id||i} style={{ padding:12, background:i%2===0?'#fafafa':'#fff', borderRadius:10, marginBottom:8, border:`1px solid ${C.border}` }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                    ...
                    {isAdmin&&<div style={{ display:'flex', gap:6 }}><Btn variant="secondary" small onClick={()=>openEdit(item)}>Edit</Btn><Btn variant="danger" small onClick={()=>deleteMealItem(item)}>Del</Btn></div>}
                  </div>
                  ...
                  <tbody>
                    {items.map((item,i)=>(
                      <tr key={item.id||i} style={{ background:i%2===0?'#fafafa':'#fff' }}>
                        ...
                        {isAdmin&&<td style={S.td}><div style={{ display:'flex', gap:5 }}><Btn variant="secondary" small onClick={()=>openEdit(item)}>Edit</Btn><Btn variant="danger" small onClick={()=>deleteMealItem(item)}>Del</Btn></div></td>}
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div style={S.card}>
        ...
        <div style={{ overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
          <table style={S.table}>
            <thead><tr>{['Food','Per','Cal','Pro','Carbs','Fat'].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map((f,i)=>(
...
function RoadmapPage({ user, isAdmin }) {
  ...
  const saveMonth=async()=>{
    ...
    try {
      ...
      setFormMsg('✓ Saved'); setShowModal(false); setEditMonth(null)
    } catch(e){setFormMsg(`Error: ${e.message}`)}
  }

  const clearPhase=async(r)=>{
    if (!window.confirm(`Clear phase for ${r.month_name}?`)) return
    if (isDemo||clientId==='demo'){setRoadmap(prev=>prev.map(m=>m.id===r.id?{...m,phase:null,notes:''}:m));return}
  ...
  const RoadmapForm=(
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
        <div><span style={S.lbl}>Month number (1-12)</span><input type="number" value={rmForm.month_number} onChange={e=>setRmForm(p=>({...p,month_number:e.target.value}))} min="1" max="12" style={S.inp}/></div>
        ...
        <div style={{ gridColumn:'1/-1' }}><span style={S.lbl}>Notes</span><textarea value={rmForm.notes} onChange={e=>setRmForm(p=>({...p,notes:e.target.value}))} rows={3} placeholder="e.g. Started cut, holiday…" style={{ ...S.inp, resize:'vertical' }}/></div>
      </div>
      <MsgBox msg={formMsg}/>
      <div style={{ display:'flex', gap:9, marginTop:14 }}>
        <Btn onClick={saveMonth} style={{ flex:1 }}>{editMonth?'Save changes':'Add / Update month'}</Btn>
  ...
  return (
    <div style={S.page}>
      ...
      {showModal&&<Modal title={editMonth?`Edit: ${editMonth.month_name}`:'Update roadmap month'} onClose={()=>{setShowModal(false);setEditMonth(null)}}>{RoadmapForm}</Modal>}

      {/* Monthly grid — mobile optimised */}
...
function AdminPanel({ user }) {
  ...
  useEffect(()=>{
    if (isDemo) return
    const sub=supabase.channel('admin-clients').on('postgres_changes',{event:'*',schema:'public',table:'clients'},()=>loadClients()).subscribe()
    return ()=>supabase.removeChannel(sub)
  },[])
  ...
  const createClient=async()=>{
    ...
    try {
      ...
      await loadClients(); setMsg('✓ Client created! Now create their auth user in Supabase Auth.')
  ...
  return (
    <div style={S.page}>
      <div style={{ marginBottom:18 }}><div style={S.h1}>Admin</div><div style={{ fontSize:13, color:C.gray }}>Manage clients · {isDemo?'Demo':'Live ✓'}</div></div>
          ...
          {clients.map(c=>(
            <div key={c.id} style={{ ...S.card, marginBottom:12 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                <div><div style={{ fontWeight:700, fontSize:15 }}>{c.name}</div><div style={{ fontSize:12, color:C.gray, marginTop:2 }}>{c.email}</div></div>
      ...
      {tab==='add client'&&(
        <div style={S.card}>
          ...
          <div style={{ marginTop:12 }}><span style={S.lbl}>Gender</span><select value={newForm.gender} onChange={e=>setNewForm(p=>({...p,gender:e.target.value}))} style={{ ...S.inp, maxWidth:180 }}>{['MALE','FEMALE','OTHER'].map(g=><option key={g}>{g}</option>)}</select></div>
